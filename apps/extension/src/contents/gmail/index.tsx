...
function FloatingButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-primary to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center z-[9999] hover:scale-110"
      title="Open Mail AI"
    >
      <Mail size={24} />
    </button>
...
function AIPanel({ onClose }: { onClose: () => void }) {
  ...
  const handleSummarize = async (type: 'single' | 'thread') => {
    if (!currentEmail?.body) return;
    const content = type === 'thread' && currentEmail.threadContext
      ? `${currentEmail.threadContext}\n---\n${currentEmail.body}`
      : currentEmail.body;
    await summarize({ emailContent: content, type });
  };
  ...
  return (
    <div className="fixed top-0 right-0 h-full w-[420px] bg-white shadow-2xl z-[9999] flex flex-col animate-slide-in border-l border-gray-200">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-primary to-purple-600">
        <div className="flex items-center gap-2">
          <Mail size={20} className="text-white" />
          <span className="font-semibold text-white">Mail AI</span>
        </div>
        <button
          onClick={onClose}
...
root.render(<GmailMailAI />);

export default GmailMailAI;
